...
class Constants:
    ...
    def get_constant_from_key_path(self, yaml_path, key_path):
        if not key_path in self.constants.keys():
            self.constants[key_path] = yaml_settings(yaml_path, key_path)
        return self.constants[key_path]
    def set_constant_from_key_path(self, yaml_path, key_path, value):
        self.constants[key_path] = value
        yaml_settings(yaml_path, key_path, value)

constants = Constants()

# GLOBALS ========================================
constants.vr = ""  # Used for checking VR Mode yaml setting.
# ================================================
constants.game = "Fallout4"  # Set game managed by CLASSIC.
# ================================================
...
def classic_generate_files():  # Other paths will be auto generated by the code.
    if not os.path.exists("CLASSIC Ignore.yaml"):
        default_ignorefile = constants.get_constant_from_key_path("CLASSIC Data/databases/CLASSIC Main.yaml", "CLASSIC_Info.default_ignorefile")
        with open('CLASSIC Ignore.yaml', 'w', encoding='utf-8') as file:
            file.write(default_ignorefile)

    if not os.path.exists(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml"):
        default_yaml = constants.get_constant_from_key_path("CLASSIC Data/databases/CLASSIC Main.yaml", "CLASSIC_Info.default_localyaml")
        with open(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml", "w", encoding="utf-8", errors="ignore") as file:
            file.write(default_yaml)

    if not os.path.exists(f"CLASSIC Data/databases/{constants.game} FID Mods.txt"):
        default_fidfile = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", "Default_FIDMods")
        with open(f"CLASSIC Data/databases/{constants.game} FID Mods.txt", "w", encoding="utf-8") as file:
...
def create_formid_db():
    if not os.path.exists(f"CLASSIC Data/databases/{constants.game} FormIDs.db") and os.path.exists(f"CLASSIC Data/databases/{constants.game} FID Main.txt"):
        with sqlite3.connect(f"CLASSIC Data/databases/{constants.game} FormIDs.db") as conn, open(f"CLASSIC Data/databases/{constants.game} FID Main.txt", encoding="utf-8", errors="ignore") as f:
            conn.execute(f'''CREATE TABLE IF NOT EXISTS {constants.game} 
                (id INTEGER PRIMARY KEY AUTOINCREMENT,  
                plugin TEXT, formid TEXT, entry TEXT)''')
            conn.execute(f"CREATE INDEX IF NOT EXISTS Fallout4_index ON {constants.game}(formid, plugin COLLATE nocase);")
            ...
            if len(lines) > 0:
                ...
                for line in lines:
                    ...
                    if "|" in line and len(line.split(" | ")) >= 3:
                        ...
                        conn.execute(f'''INSERT INTO {constants.game} (plugin, formid, entry) VALUES (?, ?, ?)''', (plugin, formid, entry))
                if conn.in_transaction:
                    conn.commit()
                print(" Done!")
...
def classic_data_extract():
    ...
    if not os.path.exists(f"CLASSIC Data/databases/{constants.game} FID Main.txt"):
        if os.path.exists("CLASSIC Data/CLASSIC Data.zip"):
            with zipfile.ZipFile("CLASSIC Data/CLASSIC Data.zip", "r") as zip_data:
                zip_data.extract(f"databases/{constants.game} FID Main.txt", "CLASSIC Data")
...
def classic_settings(setting=None):
    if not os.path.exists("CLASSIC Settings.yaml"):
        default_settings = constants.get_constant_from_key_path("CLASSIC Data/databases/CLASSIC Main.yaml", "CLASSIC_Info.default_settings")
        with open('CLASSIC Settings.yaml', 'w', encoding='utf-8') as file:
            file.write(default_settings)
    if setting:
        get_setting = constants.get_constant_from_key_path("CLASSIC Settings.yaml", f"CLASSIC_Settings.{setting}")
...
def classic_update_check():
    ...
    if classic_settings("Update Check"):
        classic_local = constants.get_constant_from_key_path("CLASSIC Data/databases/CLASSIC Main.yaml", "CLASSIC_Info.version")
        ...
        try:
            ...
            if classic_ver_received == classic_local:
                ...
                print(constants.get_constant_from_key_path("CLASSIC Data/databases/CLASSIC Main.yaml", f"CLASSIC_Interface.update_warning_{constants.game}"))
        except (ValueError, OSError, requests.exceptions.RequestException) as err:
            print(err)
            print(constants.get_constant_from_key_path("CLASSIC Data/databases/CLASSIC Main.yaml", f"CLASSIC_Interface.update_unable_{constants.game}"))
    else:
...
def docs_path_find():
    ...
    game_sid = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", f"Game{constants.vr}_Info.Main_SteamID")
    docs_name = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", f"Game{constants.vr}_Info.Main_Docs_Name")
    ...
    def get_windows_docs_path():
        ...
        constants.set_constant_from_key_path(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml", f"Game{constants.vr}_Info.Root_Folder_Docs", f"{win_docs}")
        ...
        if libraryfolders_path.is_file():
            ...
            for library_line in steam_library:
                ...
                if str(game_sid) in library_line:
                    ...
                    constants.set_constant_from_key_path(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml", f"Game{constants.vr}_Info.Root_Folder_Docs", f"{linux_docs}")
    ...
    def get_manual_docs_path():
        ...
        while True:
            ...
            if os.path.exists(path_input) and os.path.isdir(path_input):
                ...
                constants.set_constant_from_key_path(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml", f"Game{constants.vr}_Info.Root_Folder_Docs", manual_docs)
                break
            else:
...
def game_path_find():
    ...
    xse_file = constants.get_constant_from_key_path(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml", f"Game{constants.vr}_Info.Docs_File_XSE")
    xse_acronym = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", f"Game{constants.vr}_Info.XSE_Acronym")
    game_name = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", f"Game{constants.vr}_Info.Main_Root_Name")
        ...
        with open(xse_file, "r", encoding="utf-8", errors="ignore") as LOG_Check:
            ...
            for logline in Path_Check:
                if "plugin directory" in logline:
                    ...
                    constants.set_constant_from_key_path(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml", f"Game{constants.vr}_Info.Root_Folder_Game", str(game_path))
    else:
...
def game_check_integrity() -> str:
    ...
    steam_ini_local = constants.get_constant_from_key_path(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml", f"Game{constants.vr}_Info.Game_File_SteamINI")
    exe_hash_old = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", f"Game{constants.vr}_Info.EXE_HashedOLD")
    # exe_hash_new = yaml_settings(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", f"Game{constants.vr}_Info.EXE_HashedNEW") | RESERVED FOR 2023 UPDATE
    game_exe_local = constants.get_constant_from_key_path(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml", f"Game{constants.vr}_Info.Game_File_EXE")
    root_name = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", f"Game{constants.vr}_Info.Main_Root_Name")
    ...
    if game_exe_path.is_file():
        ...
        if "Program Files" not in str(game_exe_path):
            ...
            root_warn = constants.get_constant_from_key_path("CLASSIC Data/databases/CLASSIC Main.yaml", "Warnings_GAME.warn_root_path")
...
def xse_check_integrity() -> str:  # RESERVED | NEED VR HASH/FILE CHECK
    ...
    catch_errors = constants.get_constant_from_key_path("CLASSIC Data/databases/CLASSIC Main.yaml", "catch_log_errors")
    xse_acronym = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", f"Game{constants.vr}_Info.XSE_Acronym")
    ...
    xse_full_name = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", f"Game{constants.vr}_Info.XSE_FullName")
    ...
    adlib_file = constants.get_constant_from_key_path(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml", f"Game{constants.vr}_Info.Game_File_AddressLib")
    ...
    match adlib_file:
        case str() | Path():
            if Path(adlib_file).exists():
                ...
                message_list.append(constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", "Warnings_MODS.Warn_ADLIB_Missing"))
        case _:
            message_list.append(f"❌ Value for Address Library is invalid or missing from CLASSIC {constants.game} Local.yaml!\n-----\n")
    ...
    match xse_log_file:
        case str() | Path():
            if Path(xse_log_file).exists():
                ...
                if str(xse_ver_latest) in xse_data[0]:
                    ...
                    message_list.append(constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", "Warnings_XSE.Warn_Outdated"))
        ...
        case _:
            message_list.append(f"❌ Value for {xse_acronym.lower()}.log is invalid or missing from CLASSIC {constants.game} Local.yaml!\n-----\n")
...
def xse_check_hashes() -> str:
    ...
    xse_hashedscripts = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", f"Game{constants.vr}_Info.XSE_HashedScripts")
    game_folder_scripts = constants.get_constant_from_key_path(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml", f"Game{constants.vr}_Info.Game_Folder_Scripts")
    ...
    if xse_script_missing:
        message_list.append(constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", "Warnings_XSE.Warn_Missing"))
    if xse_script_mismatch:
        message_list.append(constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", "Warnings_XSE.Warn_Mismatch"))
...
def docs_check_folder():
    ...
    if "onedrive" in docs_name.lower():
        docs_warn = constants.get_constant_from_key_path("CLASSIC Data/databases/CLASSIC Main.yaml", "Warnings_GAME.warn_docs_path")
...
def docs_check_ini(ini_name) -> str:
    ...
    folder_docs = constants.get_constant_from_key_path(f"CLASSIC Data/CLASSIC {constants.game} Local.yaml", f"Game{constants.vr}_Info.Root_Folder_Docs")
    docs_name = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", f"Game{constants.vr}_Info.Main_Docs_Name")
    ...
    if any(ini_name.lower() in file.name.lower() for file in ini_file_list):
        ...
        if ini_name.lower() == f"{docs_name.lower()}custom.ini":
            with open(ini_path, "a", encoding="utf-8", errors="ignore") as ini_file:
                message_list.extend(["❌ WARNING : Archive Invalidation / Loose Files setting is not enabled. \n",
                                     "  CLASSIC will now enable this setting automatically in the game INI files. \n-----\n"])
                customini_config = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", "Default_CustomINI")
...
def main_files_backup():
    ...
    if xse_links:
        ...
        if not any(version_format in link for link in xse_links):
            print(constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", "Warnings_XSE.Warn_Outdated"))
...
def main_combined_result():
    ...
    combined_return = [game_check_integrity(), xse_check_integrity(), xse_check_hashes(), docs_check_folder(),
                       docs_check_ini(f"{constants.game}.ini"), docs_check_ini(f"{constants.game}Custom.ini"), docs_check_ini(f"{constants.game}Prefs.ini")]
...
def main_generate_required():
    ...
    classic_ver = constants.get_constant_from_key_path("CLASSIC Data/databases/CLASSIC Main.yaml", "CLASSIC_Info.version")
    game_name = constants.get_constant_from_key_path(f"CLASSIC Data/databases/CLASSIC {constants.game}.yaml", "Game_Info.Main_Root_Name")
